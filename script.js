...
function speakInvoiceSummary() {
    ...
    const summary = `Invoice summary. From ${businessName} to ${customerName}. Total amount: ${total}. Thank you for using BillNeXX!`;
    speakText(summary);
...
function openPhonePe() {
    ...
    const upiId = "ravikasaudhan317186.rzp@rxairtel"; // APNA PAYTM UPI ID
    const url = `phonepe://pay?pa=${upiId}&pn=BillNeXX&am=${amount}&cu=INR`;
    window.location.href = url;
}

function openGooglePay() {
    const amount = document.getElementById('paymentAmount').textContent;
    const upiId = "9672932630@ptaxis"; // SAME PAYTM UPI ID
    const url = `tez://upi/pay?pa=${upiId}&pn=BillNeXX&am=${amount}&cu=INR`;
    window.location.href = url;
}
...
function openPaytm() {
    const amount = document.getElementById('paymentAmount').textContent;
    const upiId = "ravikasaudhan317186.rzp@rxairtel"; // APNA PAYTM UPI ID
    const url = `paytmmp://pay?pa=${upiId}&pn=BillNeXX&am=${amount}&cu=INR`;
    window.location.href = url;
}
...
function openAmazonPay() {
    ...
    const url = `amazonpay://pay?pa=${upiId}&pn=BillNeXX&am=${amount}&cu=INR`;
    window.location.href = url;
}
...
// Update Cart Icon in Header - SIMPLE VERSION
function updateCartIcon() {
    const totalItems = shoppingCart.reduce((sum, item) => sum + item.quantity, 0);
    const cartCountElement = document.getElementById('flipkartCartCount');
    if (cartCountElement) {
...
function openSignupPage() {
    window.open('triple-login.html', 'BillNeXX Signup', 'width=500,height=700,scrollbars=no,resizable=no');
}

// Handle successful events from popup
...
function loadUserData() {
    const savedUser = localStorage.getItem('billnexx_user');
    const savedProfile = localStorage.getItem('billnexx_profile');
    const savedInvoices = localStorage.getItem('billnexx_invoices');
    const savedCustomers = localStorage.getItem('billnexx_customers');
...
function saveUserData() {
    localStorage.setItem('billnexx_profile', JSON.stringify(userProfile));
    localStorage.setItem('billnexx_invoices', JSON.stringify(userProfile.invoices));
    localStorage.setItem('billnexx_customers', JSON.stringify(userProfile.customers));
}
                ...
                <div class="profile-menu">
                    ...
                    <div class="profile-menu-section">
                        ...
                        <a href="#" onclick="openSparkPayModal(); closeProfileMenu();">
                            <i class="fas fa-wallet"></i> BillNeXX Pay
...
function loadInvoiceHistory() {
    ...
    const paymentData = JSON.parse(localStorage.getItem('billnexx_payments') || '{"customers":[]}');
    const oldInvoices = JSON.parse(localStorage.getItem('billnexx_invoices') || '[]');
...
// Loan Data Structure
let loanCustomers = JSON.parse(localStorage.getItem('billnexx_loans') || '[]');
...
function loadCustomerHistory() {
    ...
    const paymentData = JSON.parse(localStorage.getItem('billnexx_payments') || '{"customers":[]}');
    const oldInvoices = JSON.parse(localStorage.getItem('billnexx_invoices') || '[]');
    const loanCustomers = JSON.parse(localStorage.getItem('billnexx_loans') || '[]');
...
function saveLoanData() {
    localStorage.setItem('billnexx_loans', JSON.stringify(loanCustomers));
}
...
function loadBusinessDetails() {
    const savedBusiness = localStorage.getItem('billnexx_business');
    if (savedBusiness) {
        try {
...
function saveBusinessDetails() {
    try {
        ...
        localStorage.setItem('billnexx_business', JSON.stringify(businessDetails));
...
function saveInvoiceData() {
    ...
    // Save to localStorage
    localStorage.setItem('billnexx_payments', JSON.stringify(paymentData));
...
function updateBusinessStats() {
    try {
        ...
        const businessDetails = JSON.parse(localStorage.getItem('billnexx_business') || '{"createdAt":"2024-01-01"}');
...
function initPaymentSystem() {
    ...
    const savedPayments = localStorage.getItem('billnexx_payments');
    if (savedPayments) {
...
// BillNeXX SMART AI Chatbot - TEXT + VOICE COMPLETE
document.addEventListener('DOMContentLoaded', function() {
    console.log('🎯 BillNeXX SMART AI Initializing...');
    ...
    const textSpeakBtn = document.getElementById('text-speak-btn');
    ...
    function initializeVoiceRecognition() {
        if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
            ...
            recognition.onstart = function() {
                ...
                voiceInputBtn.innerHTML = '<i class="fas fa-square"></i>';
                modeIndicator.textContent = '🎤 Listening... Bolte rahiye!';
            ...
            recognition.onresult = function(event) {
                ...
                setTimeout(() => {
                    sendMessage();
                    modeIndicator.textContent = '📝 Processing your voice...';
                }, 1000);
            };
            ...
            recognition.onerror = function(event) {
                ...
                if (event.error === 'not-allowed') {
                    addMessage('❌ Microphone permission denied. Please allow microphone access to use voice commands.', 'bot');
            ...
            addMessage('ℹ️ Voice recognition is not supported in your browser. Please use Chrome or Edge.', 'bot');
        }
    }
    ...
    function stopVoiceRecognition() {
        ...
        modeIndicator.textContent = '📝 Text Mode - Type karo ya voice se bolo';
    }
    ...
    function speakText(text, rate = 0.9, pitch = 1.0) {
        ...
        const englishVoice = voices.find(voice => 
            voice.lang.includes('en') || voice.name.includes('Google UK English Male')
        );
        
        if (hindiVoice && text.includes('ह')) {
        ...
        currentUtterance.onstart = function() {
            ...
            isSpeaking = true;
            textSpeakBtn.classList.add('speaking');
            textSpeakBtn.innerHTML = '<i class="fas fa-stop"></i>';
            modeIndicator.textContent = '🔊 Speaking...';
        };

        currentUtterance.onend = function() {
            console.log('🔊 Finished speaking');
            isSpeaking = false;
            textSpeakBtn.classList.remove('speaking');
            textSpeakBtn.innerHTML = '<i class="fas fa-volume-up"></i>';
            modeIndicator.textContent = '📝 Text Mode - Type karo ya voice se bolo';
        };
    ...
    function initializeChatbot() {
        ...
        toggleBtn.addEventListener('click', function(e) {
            ...
            console.log('Chatbot is now:', chatbot.classList.contains('active') ? 'OPEN' : 'CLOSED');
        ...
        voiceInputBtn.addEventListener('click', function() {
            if (!recognition) {
                addMessage('❌ Voice recognition is not available in your browser.', 'bot');
                return;
            }
        ...
        textSpeakBtn.addEventListener('click', function() {
            if (isSpeaking) {
                stopSpeaking();
            } else {
                const lastBotMessage = document.querySelector('.bot-message:last-child');
                if (lastBotMessage) {
                    const messageText = lastBotMessage.querySelector('.message-content p').textContent;
                    speakText(messageText);
                } else {
                    addMessage('❌ No message to speak. Please chat with me first!', 'bot');
                }
            }
        });
        ...
        input.addEventListener('focus', function() {
            modeIndicator.textContent = '📝 Typing Mode - Type your message';
        });

        // 9. WINDOW CLICK (Close chatbot when clicking outside)
        document.addEventListener('click', function(e) {
            if (!chatbot.contains(e.target) && !toggleBtn.contains(e.target) && chatbot.classList.contains('active')) {
                chatbot.classList.remove('active');
                stopVoiceRecognition();
                stopSpeaking();
            }
        ...
        console.log('✅ BillNeXX SMART AI Ready!');
    }
    ...
    function sendMessage() {
        ...
        // Bot thinking with delay for realistic typing
        setTimeout(() => {
            ...
            // Auto-speak if text-to-speech was active
            if (textSpeakBtn.classList.contains('active')) {
    ...
    // Advanced Bot Response Logic - Hindi + English Mixed
    function getAdvancedBotResponse(message) {
        const lowerMsg = message.toLowerCase();
        
        // Greetings & Casual Chat
        if (/(hello|hi|hey|namaste|kaise ho|how are you|good morning|good afternoon|good evening)/i.test(lowerMsg)) {
            const greetings = [
                "👋 **Namaste! Main BillNeXX AI hu!**\n\nAap kaise hain? Main aapki billing aur business management mein help kar sakta hun! 😊\n\n*Hello! I'm BillNeXX AI! How are you? I can help you with billing and business management!*",
                
                "🙏 **Pranam! Aapka din accha ja raha hai?**\n\nMujhse billing, invoices, payments, ya koi bhi business related help le sakte hain! 🚀\n\n*Greetings! Hope you're having a good day! You can ask me about billing, invoices, payments or any business help!*",
                
                "😊 **Hello there! Main yahan hun aapki help ke liye!**\n\nAap kya kar rahe hain aaj? Koi business problem hai ya just chat karna chahte hain? 💬\n\n*Hello there! I'm here to help you! What are you doing today? Any business problem or just want to chat?*"
            ];
            return greetings[Math.floor(Math.random() * greetings.length)];
        }

        // Invoice Related
        if (/(invoice|bill|बिल|इनवॉइस|बनाना|create|generate)/i.test(lowerMsg)) {
            return "🎯 **Invoice Creation Help:**\n\n**English:**\n• Voice Command: 'Create invoice for Rajesh'\n• Add Items: 'Add laptop 50000'\n• Generate PDF: 'Download bill'\n• Share: 'Share on WhatsApp'\n\n**Hindi:**\n• वॉइस कमांड: 'राजेश के लिए इनवॉइस बनाओ'\n• आइटम जोड़ो: 'लैपटॉप 50000 जोड़ो'\n• पीडीएफ डाउनलोड: 'बिल डाउनलोड करो'\n• शेयर करो: 'व्हाट्सएप्प पर भेजो'\n\n💡 *Pro Tip: Voice commands se 2x faster kaam karein!*";
        }

        // Payment Related
        if (/(payment|pay|पेमेंट|भुगतान|कैश|upi|card|credit)/i.test(lowerMsg)) {
            return "💳 **Payment Methods Guide:**\n\n**Available Options:**\n• Cash - नकद\n• UPI - यूपीआई (PhonePe, GPay, Paytm)\n• Card - कार्ड (Debit/Credit)\n• Credit - उधार (Udhaar)\n• Bank Transfer - बैंक ट्रांसफर\n\n**Voice Commands:**\n'Payment cash', 'Payment UPI', 'Payment udhaar'\n'भुगतान नकद', 'भुगतान यूपीआई', 'उधार दर्ज करो'";
        }

        // Technical Issues
        if (/(problem|error|issue|समस्या|त्रुटि|not working|ठीक नहीं)/i.test(lowerMsg)) {
            return "🔧 **Technical Support:**\n\n**Common Solutions:**\n• Voice Not Working? → Microphone permissions check karein\n• PDF Download Issue? → Chrome browser use karein\n• Data Lost? → Local storage check karein\n• Mobile App? → Browser use karein (PWA supported)\n\n**समाधान:**\n• माइक्रोफोन परमिशन चेक करें\n• क्रोम ब्राउज़र इस्तेमाल करें\n• डेटा बैकअप चेक करें\n• मोबाइल पर ब्राउज़र इस्तेमाल करें\n\n📞 Need more help? Contact our support team!";
        }

        // Business Setup
        if (/(business|shop|व्यवसाय|दुकान|setup|start|शुरू)/i.test(lowerMsg)) {
            return "🏪 **Business Setup Guide:**\n\n**Step-by-Step:**\n1. Business Details: 'Setup business Sharma Traders'\n2. Auto-fill: Enable auto-fill in settings\n3. GST Setup: 'Add GST number 07ABCDE1234F1Z5'\n4. Bank Details: 'Add bank account number'\n\n**हिंदी में:**\n1. 'व्यवसाय सेटअप करो शर्मा ट्रेडर्स'\n2. 'ऑटो-फिल चालू करो'\n3. 'जीएसटी नंबर डालो'\n4. 'बैंक खाता जोड़ो'\n\n🚀 *30-minute mein business ready!*";
        }

        // Voice Commands Help
        if (/(voice|speak|बोल|आवाज|कमांड|command)/i.test(lowerMsg)) {
            return "🎤 **Voice Commands Master Guide:**\n\n**English Commands:**\n'Create invoice', 'Add item laptop', 'Set price 5000', 'Calculate total', 'Download PDF', 'Save customer', 'Show reports'\n\n**Hindi Commands:**\n'इनवॉइस बनाओ', 'आइटम जोड़ो लैपटॉप', 'कीमत सेट करो 5000', 'टोटल कैलकुलेट करो', 'पीडीएफ डाउनलोड करो', 'ग्राहक सेव करो', 'रिपोर्ट दिखाओ'\n\n🎯 *Voice use karke 60% time bachayein!*";
        }

        // Pricing & Plans
        if (/(price|cost|plan|मूल्य|योजना|कीमत|पैसा)/i.test(lowerMsg)) {
            return "💰 **Pricing Plans - Best Value:**\n\n✨ **FREE Plan** ✨\n• Basic invoices\n• Voice commands\n• 3 templates\n• Mobile support\n\n🚀 **PRO Plan** ($12/month)\n• Advanced features\n• 50+ templates\n• Priority support\n• Custom branding\n\n🏢 **BUSINESS Plan** ($29/month)\n• Team collaboration\n• Unlimited templates\n• Dedicated support\n• API access\n\n🎁 **30-day FREE trial for all plans!**";
        }

        // Casual Conversation
        if (/(chat|talk|baat|बातचीत|conversation|time pass)/i.test(lowerMsg)) {
            const casualResponses = [
                "😊 **Of course! Main yahan hun baat karne ke liye!**\n\nAapka din kaisa chal raha hai? Koi interesting project par kaam kar rahe hain?\n\n*Of course! I'm here to chat! How's your day going? Working on any interesting projects?*",
                
                "💬 **Baat-cheet acchi lagti hai!** 🤗\n\nMujhe technology, business, startups, ya fir aapke plans ke bare mein baat karna accha lagta hai!\n\n*I love conversations! I enjoy talking about technology, business, startups, or your plans!*",
                
                "🎯 **Let's chat! Main ready hun!**\n\nAap kya discuss karna chahenge? Technology, business strategies, life goals, ya kuch aur?\n\n*Let's chat! I'm ready! What would you like to discuss? Technology, business strategies, life goals, or something else?*"
            ];
            return casualResponses[Math.floor(Math.random() * casualResponses.length)];
        }

        // Emotional Support
        if (/(sad|upset|tension|problem|help|मदद|परेशान|चिंता)/i.test(lowerMsg)) {
            return "🤗 **I understand... Aap tension mein lag rahe hain.**\n\nYaad rakhiye, har problem ka solution hota hai. Main yahan hun aapki help ke liye!\n\nAap specifically kya problem face kar rahe hain? Business related hai ya personal?\n\n*I understand... You seem worried. Remember, every problem has a solution. I'm here to help you! What specific problem are you facing? Business related or personal?*";
        }

        // Default Smart Response
        return `🤔 **Main samjha aap puchh rahe hain: "${message}"**\n\n*I understand you're asking about: "${message}"*\n\n**Mai aapki kis tarah help kar sakta hun?**\n• Invoice & Billing - बिलिंग और इनवॉइस\n• Business Setup - व्यवसाय सेटअप\n• Technical Support - तकनीकी सहायता\n• Payment Solutions - भुगतान समाधान\n• Casual Chat - आम बातचीत\n\n**How can I help you with this?**\nCan you provide more details? क्या आप और details de sakte hain?\n\n💡 *Try: "Invoice kaise banaye?" ya "Technical problem hai"*`;
    }
    ...
    function addMessage(content, sender) {
        ...
        messageDiv.innerHTML = `
            <div class="message-avatar">
                ${sender === 'bot' ? 
                    '<div class="mini-robot"><div class="robot-face"><div class="eye left-eye"></div><div class="eye right-eye"></div></div></div>' : 
            ...
            <div class="message-content">
                ...
                ${sender === 'bot' ? `
                    <div class="voice-controls">
                        <button class="voice-play-btn" onclick="window.speakLastMessage()">
                            <i class="fas fa-play"></i> Ye message suno / Hear this
                        </button>
                    </div>
                ` : ''}
    ...
    setTimeout(() => {
        const firstTime = !localStorage.getItem('chatbot_visited');
        if (firstTime) {
            ...
            setTimeout(() => {
                addMessage("👋 <strong>Namaste! Main BillNeXX SMART AI hu!</strong>\n\nAap mujhse:\n• <strong>Type karke</strong> baat kar sakte hain\n• <strong>Voice mein</strong> bhi baat kar sakte hain\n• Main bhi <strong>voice mein bol sakta</strong> hun\n\n💬 <em>Kuch bhi puchho - billing, business, ya fir just chat!</em>\n\n<strong>Hello! I'm BillNeXX SMART AI!</strong>\nYou can:\n• Chat by <strong>typing</strong>\n• Talk using <strong>voice</strong>\n• I can <strong>speak responses</strong> too\n\n💬 <em>Ask me anything - billing, business, or just chat!</em>", 'bot');
            }, 1000);
        }
    }, 2000);
...
// Global function for voice play button
window.speakLastMessage = function() {
    ...
    if (botMessages.length > 0) {
        ...
        // Find speech synthesis
        const speechSynthesis = window.speechSynthesis;
        if (speechSynthesis) {
            const utterance = new SpeechSynthesisUtterance(messageText.substring(0, 300));
            utterance.rate = 0.9;
            utterance.pitch = 1.0;
